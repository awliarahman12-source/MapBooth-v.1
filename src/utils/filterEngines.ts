 ...
 * Filter Engines
 * 
 * Kumpulan engine untuk filter tipe 'canvas' — manipulasi pixel
 * via Canvas API.
 */

export type EngineName = 'swirl' | 'twirl' | 'pixelate' | 'fisheye' | 'wave';
...
function applyPixelate(
  ...
  for (let by = 0; by < h; by += size) {
    for (let bx = 0; bx < w; bx += size) {
      ...
      const maxY = Math.min(by + size, h);
      const maxX = Math.min(bx + size, w);
